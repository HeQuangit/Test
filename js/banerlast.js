		...
		var $btn_left = $(".btnleft").children();
		var $btn_rigth = $(".btnrigth").children();
		...
		function leftb(){
			...
			if(!$obj_ul.is(":animated")){
				...
				if(numtime < numlen - 1){
					numtime ++ ;	//信号量加1
					//拉到信号量的倍数上：
					$obj_ul.animate({"left":-256 * numtime+"px"},500);
				}else{
					//信号量到了最大，让信号量变为0
					numtime = 0;
					//先往猫腻上移动，然后瞬间移动到0
					$obj_ul.animate({"left":-256 * numlen+"px"},500,function(){
						$(this).css("left","0px");
					});
				}
			}
